...
> **Current version: v1.3.0 (Commercial GA)** — fusion-identity integration landed (issue #18): fm-server boundary enforces caller JWT, tid↔tenant match, empty-tenant rejection in multi-tenant mode. v1.2.0 multi-tenant isolation landed (issue #16). Known limitations in `CHANGELOG.md` "Known limitations", non-blocking.
...
**fusion-identity integration landed (2026-09-03, issue #18)**: fm-server HTTP boundary enforces caller JWT verified by fusion-identity, tid↔tenant match, and empty-tenant rejection in multi-tenant mode (multi-tenant PRD §4 red line 1 fail-closed). Single-tenant backward compatible (default `multi_tenant: false` = no identity verify).
- **Identity verifier** (`fm-server/src/identity.rs`): `IdentityVerifier` trait (`verify` + `report_usage`) for 100% offline testability. `RealVerifier` (reqwest to `127.0.0.1` only, 30s TTL result cache) — fail-closed on HTTP error. `NoopVerifier` for single-tenant backward compat. `enforce_multi_tenant`: `multi_tenant=false` short-circuits; true + empty tenant → 401; missing Bearer JWT → 401; verify → tid != tenant → 403; match → Ok(tid).
- **Config** (`fm-server/src/config.rs`): `multi_tenant: bool` (default false), `identity_url` (default `http://127.0.0.1:11470`), `identity_service_token` (+ `_file` secret). Env: `FUSION_MEMORY_MULTI_TENANT` / `FUSION_MEMORY_IDENTITY_URL` / `FUSION_MEMORY_IDENTITY_SERVICE_TOKEN` / `FUSION_MEMORY_IDENTITY_SERVICE_TOKEN_FILE`. `validate()`: `multi_tenant=true` requires http identity_url AND non-empty service_token.
- **HTTP boundary** (`fm-server/src/http.rs`): in multi-tenant mode `check_bearer` (static api_key) is skipped — Bearer is the JWT (identity-verified). `handle_rpc` + `get_memory` call `enforce_multi_tenant` after tenant extraction.
- **Test counts**: default features 433→452 (+19 identity tests: 4 config + 7 http + 8 identity unit/e2e); `--features fm-store/store-fusion` 439→458. Gates all green (fmt / clippy -D warnings / check / test). Version bump all 12 crates 1.2.0→1.3.0 (additive, no breaking change).
- **Upstream**: fusion-identity consumed (not modified) — sole JWT issuer + tenant registry. fm-server calls `POST /api/v1/auth/verify` (service-token gated) + `POST /api/v1/tenants/{tenant_id}/usage` (best-effort). Three red lines honored: fail-closed, cross-tenant denied, data isolation layering.
