...
//! 当前 fusion-store trait API 对不上 + 跨工程约束 (见 README "M5 PRD 偏离记录"), 暂用 store-stub
...
//!   - 接 fusion-store HNSW 索引 (零拷贝 mmap, 替 store-stub 的 hnsw_rs+sled owned)
//!   - ZeroCopyBuffer.data 换 mmap 切片 (真零拷贝, 见 trait_def §3.16 注释)
//!   - 走 issue→PR 流程落地 (遵循 monorepo 跨工程约束)

#[cfg(not(feature = "store-stub"))]
compile_error!(
    "store-fusion backend not yet implemented: fusion-store upstream trait alignment pending. \
     Build with default features (store-stub) instead. See README \"M5 PRD 偏离记录\" + fusion-memory §1.4."
);
