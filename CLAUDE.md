...
**Built — M0 through M6 landed. v1.3.0 released (tag `v1.3.0`, issue #18 fusion-identity integration); v1.2.0 multi-tenant isolation (tag `v1.2.0`, issue #16); v1.1.0 Commercial GA released (tag `v1.1.0`).** Rust Cargo workspace, 12 crates (`fm-cli`/`fm-core`/`fm-embed`/`fm-engine`/`fm-fuzz`/`fm-graph`/`fm-persist`/`fm-py`/`fm-server`/`fm-similarity`/`fm-store`/`fm-cluster`). 452 offline tests green (458 with `--features fm-store/store-fusion`), regions 90.82% offline / 92.47% live. Milestone detail + PRD deviation records in `README.md`. v1.3.0: fusion-identity integration (fm-server HTTP boundary verifies caller JWT via `POST /api/v1/auth/verify`, enforces tid↔tenant match → 403 on mismatch, rejects empty tenant in multi-tenant mode → 401 fail-closed, reports usage; `multi_tenant` config default false = single-tenant backward compat, no identity verify; `IdentityVerifier` trait + `RealVerifier`/`NoopVerifier` for 100% offline testability; config `multi_tenant`/`identity_url`/`identity_service_token` + env `FUSION_MEMORY_MULTI_TENANT`/`FUSION_MEMORY_IDENTITY_URL`/`FUSION_MEMORY_IDENTITY_SERVICE_TOKEN`). v1.2.0: multi-tenant isolation (tenant field on data, additive tenant-scoped trait methods, schema v3, gateway-origin enforcement `X-Fusion-Route`/`X-Fusion-Tenant`, config `gateway_origin_required`/`default_tenant`). RC known limitations all closed in 1.1.0: live perf baseline (real bge-m3), store-fusion adapter, fg-redact credentials. Remaining non-blocking: M5 store-fusion switch + guard DLP gate degraded (see README "M5 PRD 偏离记录"); PII stays fusion-memory-local pending fusion-guard #13. M4 outward integration PRs: `fusion-cowork` #68 + `fusion-agent-studio` #247 merged; `fusion-code` #151 merged / #156 open.
...
cargo test --workspace                             # All offline tests (452 cases, excludes fm-py cdylib; --features fm-store/store-fusion = 458)
